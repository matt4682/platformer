#include "./boolean_option.h"

namespace GUI {
  BoolOption::BoolOption(std::string name, FontManager &fontManager) : Option(name, fontManager), mOptionPairs() {
  }
  ...
  void BoolOption::select(std::size_t index) {
    ...
    mValueText.setOrigin(mValueText.getGlobalBounds().width / 2.f, mNameText.getGlobalBounds().height / 2.f);
  }
