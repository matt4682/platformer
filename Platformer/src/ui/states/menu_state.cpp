...
MenuState::MenuState(StateStack &stack, Context context) :
  ...
  mGrass = sf::Sprite(groundTexture, sf::IntRect(0, 0, static_cast<int>(groundTexture.getSize().x * numRepeats * 2), groundTexture.getSize().y));
  float yOffset = mGrass.getGlobalBounds().height;
  mGrass.setPosition(0, windowSize.y - yOffset);

  sf::Texture &bgTexture = context.textureManager->get(TextureID::GrasslandsBackground);
  bgTexture.setRepeated(true);
  mBackground.setTexture(bgTexture);
  mBackground.setTextureRect(sf::IntRect(0, 0, static_cast<int>(windowSize.x), static_cast<int>(windowSize.y)));
  ...
  playButton->setCallback(
    [this]() {
    requestStackPop();
    requestStackPush(ID::WorldSelect, "Tutorial");
  });
  playButton->setText("Play", sf::Color::White);
  ...
  mTitleText.setString("Platformer");
...
bool MenuState::update(sf::Time deltaTime) {
  ...
  mGrass.move(-physics::MAX_SPEED / 1.5f * game::PX_PER_M * deltaTime.asSeconds(), 0.f);
  ...
  mBackdrop.move(-physics::MAX_SPEED * game::PX_PER_M * deltaTime.asSeconds(), 0.f);
...
void MenuState::onResolutionChange() {
  ...
  sf::Texture &bgTexture = getContext().textureManager->get(TextureID::GrasslandsBackground);
  bgTexture.setRepeated(true);
  mBackground.setTexture(bgTexture);
