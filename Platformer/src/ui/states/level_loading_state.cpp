...
LevelLoadingState::LevelLoadingState(StateStack &stack, Context context, const std::string &map) :
  ...
  mLoadingThread([context, map]() {
  context.levelManager->load(map);
}),
mMutex(),
mAnimation(),
mSprite(),
...
mTexturesLoaded(false) {
  ...
  //mBackground = sf::Sprite(bgTexture, sf::IntRect(0, 0, context.window->getSize().x, context.window->getSize().y));
  ...
  mSprite.setPosition(context.window->getSize().x / 2.f, context.window->getSize().y / 2.f - 50.f);

  sf::Font &font = context.fontManager->get(FontID::Main);
  mLoadingText.setFont(font);
  setLoadingText("Loading Map");

  mLoadingText.setPosition(context.window->getSize().x / 2.f, context.window->getSize().y / 2.f);
...
void LevelLoadingState::onResolutionChange() {}