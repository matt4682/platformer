...
GameObject *GameObjectFactory::createGameObject(Object &object) {
  ...
  else if (type.compare("Exit") == 0 || type.compare("ExitTop") == 0) {
    newObject = new GameObject(Category::Type::Exit, object.sprite, objectBody);
  }
...
b2Body *GameObjectFactory::createPhysicsBody(Object &object) {
  std::string type = object.type;
  ...
  bodyDef.position = b2Vec2(
    position.x / game::PX_PER_M,
    (mapHeightPixels - position.y) / game::PX_PER_M - objectSize.y / 2);
  ...
  boundingBox.SetAsBox(
    bounds.width / game::PX_PER_M / 2,
    bounds.height / game::PX_PER_M / 2 * objectSize.y);
...
b2Vec2 GameObjectFactory::getObjectSize(Object &object) {
  std::string type = object.type;

  if (type.compare("Spikes") == 0) {
    return b2Vec2(game::PX_PER_M, game::PX_PER_M * 0.5f);
  }
  if (type.compare("Water") == 0) {
    return b2Vec2(game::PX_PER_M, game::PX_PER_M);
  }
